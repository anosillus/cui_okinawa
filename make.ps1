...
#>
function lint() {
    poetry run flake8 src\cui_okinawa tests
}
...
#>
function coverage() {
    poetry run coverage run --source src\cui_okinawa -m pytest
...
#>
function docs(){
    if (Test-Path ".\docs\cui_okinawa.rst") { Remove-Item ".\docs\cui_okinawa.rst" -Force }
    if (Test-Path ".\docs\modules.rst") { Remove-Item ".\docs\modules.rst" -Force }
    poetry run sphinx-apidoc -o docs\ src\cui_okinawa
